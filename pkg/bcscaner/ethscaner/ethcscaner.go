...
func (e *Etherscan) ExecutionStatus(txHash string) (bool, error) {
	u := url.URL{
		Scheme: "https",
		Host:   e.host,
		Path:   "api",
	...
	if s.Result == nil || s.Result.IsError == "1" {
		return false, nil
	}
...
func (e *Etherscan) GetTransactionByHash(txHash string) (bcscaner.TransactionInfo, error) {
	u := url.URL{
		Scheme: "https",
		Host:   e.host,
		Path:   "api",
	...
	res, err := http.Get(u.String())
	if err != nil {
		return bcscaner.TransactionInfo{}, fmt.Errorf("error making http request: %v", err)
	}
	defer res.Body.Close()
...
func (e *Etherscan) ERC721Transfers(contractAddr, accountAddr, sort string, page, offset int) ([]bcscaner.ERC721Transfer, error) {
	u := url.URL{
		Scheme: "https",
		Host:   e.host,
		Path:   "api",
